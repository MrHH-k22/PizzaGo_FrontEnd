...
function Account() {
  ...
  const { changePasswordMutation, isChangingPassword: isPasswordSubmitting } =
    useChangePassword();
  ...
  const handleProfileSubmit = () => {
    ...
    if (!profileData.name.trim() || !profileData.email.trim()) {
      toast.error("Fullname or Email must not be empty.", {
        position: "top-right",
        autoClose: 3000,
      });
      return;
  ...
  const handlePasswordSubmit = () => {
    ...
    if (passwordData.newPassword.length < 6) {
      toast.error("Your password must have atleast 6 characters.", {
        position: "top-right",
        autoClose: 3000,
      });
      return;
    ...
    // Submit password change using the existing changePasswordMutation
